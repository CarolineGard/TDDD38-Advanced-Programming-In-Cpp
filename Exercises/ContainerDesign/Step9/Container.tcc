...
template<typename T>
Container<T> &Container<T>::operator=(std::initializer_list<T> i_list) &
{
	if (size() < i_list.size())
	{
		clear();
		start_ = allocate_(i_list.size());
		finish_ = start_ + i_list.size();
		end_of_storage_ = finish_;
	}

	pointer finish_new = start_;

	for (auto &&x : i_list)
		construct_(finish_new++, x);

	destroy_(finish_new, finish_);
	finish_ = finish_new;
	return *this;
}
...
// to initialize a Container from a list of values.
template<typename T>
Container<T>::Container(std::initializer_list<T> i_list)
{
	start_ = allocate_(i_list.size());
	finish_ = start_;
	for (const auto x : i_list)
		construct_(finish_++, x);
